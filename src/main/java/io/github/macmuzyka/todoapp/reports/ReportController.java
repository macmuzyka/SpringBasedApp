...
public class ReportController {
    ...
    ResponseEntity<TaskDoneBeforeDeadline> checkIfTaskWasDoneBeforeDeadline(@PathVariable int id) {
        return taskRepository.findById(id)
    ...
    private static class TaskDoneBeforeDeadline {
        ...
        public TaskDoneBeforeDeadline(final Task task, final List<PersistedTaskEvent> events) {
            ...
            int givenTaskEventLastIndex = events.size() - 1;
            doneBeforeDeadline = events.stream()
                    .skip(givenTaskEventLastIndex)
                    .filter(t -> t.name.equals("TaskDone"))
                    .map(t -> t.occurrence)
                    .anyMatch(taskEvent -> taskEvent.isBefore(task.getDeadline()));
        }
    }
}